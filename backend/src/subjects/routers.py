...
def read_subjects(skip: int = 0, limit: int = 10, session: Session = Depends(get_db)):
...
@level_routes.post("/")
def create_level(level: LevelCreate, session: Session = Depends(get_db)):
...
@career_routes.post("/")
def create_carrer(career: CareerCreate, session: Session = Depends(get_db)):
...
@career_routes.get("/")
def get_carrer(skip: int = 0, limit: int = 10, session: Session = Depends(get_db)):
    career_db: CareerModel = career_crud.get_all(skip, limit, session=session)

    return career_db


@career_routes.get("/carrers/{career_id}/subjects")
def get_subjects_from_career(
    career_id: int, skip: int = 0, limit: int = 10, session: Session = Depends(get_db)
):
...
@subject_routes.post("/create_example_data")
def create_example_data(session: Session = Depends(get_db)):
...
def get_subject_prerequisites(subject_id: int, session: Session = Depends(get_db)):
...
# Endpoint para obtener los sujetos de una carrera
@career_routes.get("/{career_id}")
def get_career_subjects(career_id: int, session: Session = Depends(get_db)):
